...
function pr($value) {
  ...
  {
    //open the database
    $db = new PDO('sqlite:db/comments.sqlite');

    //create the database
    $db->exec("CREATE TABLE skitter_comments (id INTEGER PRIMARY KEY, name VARCHAR, email VARCHAR, comment TEXT, created DATETIME)");    
	
	$db->exec("INSERT INTO skitter_comments (name, email, comment, created) VALUES ('João Buracão', 'joao@gmail.com.br', 'olá para todos', '".date('Y-m-d H:i:s')."');");
	
	$result = $db->query('SELECT * FROM skitter_comments');
	pr($result);
	pr($db);
